	...
	<div class="tablenav">
		...
		foreach($errorbanners as $errbanner) {
		    ...
		    <tr id='adrotateindex' class='<?php echo $errorclass; ?>'>
				...
				<td><strong><a class="row-title" href="<?php echo admin_url("/admin.php?page=adrotate-ads&view=edit&ad=".$errbanner['id']);?>" title="<?php _e('Edit', 'adrotate'); ?>"><?php echo stripslashes(html_entity_decode($errbanner['title']));?></a></strong> - <a href="<?php echo admin_url("/admin.php?page=adrotate-ads&view=report&ad=".$errbanner['id']);?>" title="<?php _e('Stats', 'adrotate'); ?>"><?php _e('Stats', 'adrotate'); ?></a><span style="color:#999;"><?php if(strlen($grouplist) > 0) echo '<br /><span style="font-weight:bold;">'.__('Groups:', 'adrotate').'</span> '.$grouplist; ?><?php if(strlen($errbanner['advertiser']) > 0) echo '<br /><span style="font-weight:bold;">'.__('Advertiser:', 'adrotate').'</span> '.$errbanner['advertiser']; ?></span></td>
			</tr>
			<?php } ?>
		</tbody>

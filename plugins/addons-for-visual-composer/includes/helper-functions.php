...
function lvca_get_taxonomy_terms_filter( $taxonomies, $chosen_terms = array() )
{
    ...
    if ( !empty($terms) && !is_wp_error( $terms ) ) {
        ...
        $output .= '<div class="lvca-filter-item segment-0 lvca-active"><a data-value="*" href="#">' . esc_html__( 'All', 'livemesh-so-widgets' ) . '</a></div>';
        $segment_count = 1;
        foreach ( $terms as $term ) {
            $output .= '<div class="lvca-filter-item segment-' . intval( $segment_count ) . '"><a href="#" data-value=".term-' . intval( $term->term_id ) . '" title="' . esc_html__( 'View all items filed under ', 'livemesh-so-widgets' ) . esc_attr( $term->name ) . '">' . esc_html( $term->name ) . '</a></div>';
            $segment_count++;
        }
        $output .= '</div>';
    }
...
{
    return array(
        'none'        => __( 'None', 'livemesh-vc-addons' ),
        'fadeIn'      => __( 'Fade In', 'livemesh-vc-addons' ),
        'fadeInLeft'  => __( 'Fade In Left', 'livemesh-vc-addons' ),
        'fadeInRight' => __( 'Fade In Right', 'livemesh-vc-addons' ),
    );
}
